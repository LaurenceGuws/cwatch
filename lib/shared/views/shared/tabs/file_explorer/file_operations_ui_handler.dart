...
class FileOperationsUiHandler {
  ...
  Future<void> handlePaste({
    ...
    for (var i = 0; i < clipboardEntries.length; i++) {
      ...
      try {
        final isSameContext =
            clipboard.contextId == service.explorerContext.id;
        if (isSameContext) {
  ...
  Future<void> handleUploadFolder({
    ...
    try {
      ...
      if (uploadEntries.isEmpty) {
        try {
          await service.ensureRemoteDirectory(baseRemotePath, createdRemoteDirs);
          successCount++;
        } catch (error) {
          failCount++;
          AppLogger().warn(
        ...
        final emptyIndex =
            uiItems.indexWhere((item) => item.label == directoryName);
        if (emptyIndex != -1) {
