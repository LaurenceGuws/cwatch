...
class _KubernetesResourcesState extends State<KubernetesResources> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final options = [
        ...
        TabChipOption(
          ...
          onSelected: () => _copyCommand('kubectl --context=${widget.contextName} --kubeconfig=${widget.configPath} top nodes'),
        ),
        TabChipOption(
          label: 'Copy `kubectl top pods -A`',
          icon: NerdIcon.copy.data,
          onSelected: () => _copyCommand('kubectl --context=${widget.contextName} --kubeconfig=${widget.configPath} top pods -A'),
        ),
      ];
  ...
  List<KubePodStat> _filterPods(KubeResourceSnapshot snapshot) {
    ...
    final pods = snapshot.pods.where((pod) {
      if (!_includeSystemNamespaces && systemNamespaces.contains(pod.namespace)) {
        return false;
      }
  ...
  void _recordHistory(KubeResourceSnapshot snapshot) {
    ...
    for (final node in snapshot.nodes) {
      ...
      record(_nodeMemHistory, node.name, (node.memoryBytes ?? 0) / (1024 * 1024));
    }
  }
  ...
  Widget _lineChartCard({
    ...
    final maxPoints = series.fold<int>(0, (acc, s) => acc = acc > s.values.length ? acc : s.values.length);
    final maxValue = maxY ??
        series.fold<double>(
          0,
              ...
              child: LineChart(
                LineChartData(
                  ...
                  gridData: FlGridData(
                    ...
                    horizontalInterval: maxValue == 0 ? 1 : (maxValue / 4).clamp(1, double.infinity),
                  ),
                  titlesData: FlTitlesData(
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: 42,
                        interval: maxValue == 0 ? 1 : (maxValue / 4).clamp(1, double.infinity),
