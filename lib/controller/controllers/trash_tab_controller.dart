...
class TrashTabController extends ChangeNotifier {
  ...
  Future<T> runShell<T>(Future<T> Function() action) async {
    try {
      return action();
    } on SshRuntimeFailure catch (error) {
      if (error.kind != SshRuntimeFailureKind.authenticationFailed) {
        rethrow;
      }
      AppLogger().warn(
        'SSH authentication failed for ${error.host.name}',
        tag: 'Trash',
        error: error,
      );
      uiAdapter.showSnackBar(
        'SSH authentication failed for ${error.host.name}. Check your key configuration in settings.',
      );
      rethrow;
